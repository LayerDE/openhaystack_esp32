...
Sha256::Sha256(const uint8_t * key, size_t keyLength) {
...
void Sha256::quick_hash(const uint8_t* in, int in_lenght, uint8_t* out){
    reset();
    memset(keyBuffer, 0, BLOCK_LENGTH);
    if (in_lenght > BLOCK_LENGTH) {
        // Hash long keys
        init();
        while (in_lenght--) write(*in++);
        memcpy(keyBuffer, result(), HASH_LENGTH);
    }
    else {
        // Block length keys are used as is
        memcpy(keyBuffer, in, in_lenght);
    }
    reset();
    memcpy(out,result(),HASH_LENGTH);
}

extern "C" void createHash(const uint8_t* in,int lenght, uint8_t* out){
    Sha256 sha_func = Sha256();
    for(int i = 0; i < lenght; i++)
        sha_func.write(in[i]);
    //for(int x = 0; x<lenght;x++)
    //    sha_func.write(in[x]);
    memcpy(out,sha_func.result(),HASH_LENGTH);
}
