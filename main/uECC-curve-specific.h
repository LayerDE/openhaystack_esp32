...
#define num_bytes_(x) (x/8+(x%8 ? 1 :0))
#define num_words_(x) (x/uECC_WORD_SIZE+(x%uECC_WORD_SIZE ? 1 :0))
#if (uECC_WORD_SIZE == 1)

#define num_words_secp160r1 num_bytes_secp160r1
#define num_words_secp192r1 num_bytes_secp192r1
#define num_words_secp224r1 num_bytes_secp224r1
#define num_words_secp256r1 num_bytes_secp256r1
#define num_words_secp256k1 num_bytes_secp256k1

#define BYTES_TO_WORDS_8(a, b, c, d, e, f, g, h) \
    0x##a, 0x##b, 0x##c, 0x##d, 0x##e, 0x##f, 0x##g, 0x##h
#define BYTES_TO_WORDS_4(a, b, c, d) 0x##a, 0x##b, 0x##c, 0x##d
#define BYTES_TO_WORDS_2(a, b) 0x##a, 0x##b
...
#define BYTES_TO_WORDS_2(a, b) 0x##b##a
...
#define BYTES_TO_WORDS_2(a, b) 0x##b##a##ull
...
static void double_jacobian_default(uECC_word_t* X1,
    ...
    uECC_word_t *t4 = malloc(num_words * uECC_WORD_SIZE);
    uECC_word_t *t5 = malloc(num_words * uECC_WORD_SIZE);
...
static void x_side_default(uECC_word_t* result, const uECC_word_t* x, uECC_Curve curve) {
    uECC_word_t *_3 = malloc(curve->num_words * uECC_WORD_SIZE); /* -a = 3 */
    wordcount_t num_words = curve->num_words;
    _3[0] = 3;
    for(int x = 1; x < num_words; x++)
        _3[x] = 0;
...
static void mod_sqrt_default(uECC_word_t* a, uECC_Curve curve) {
    bitcount_t i;
    wordcount_t num_words = curve->num_words;
    uECC_word_t *p1 = malloc(num_words * uECC_WORD_SIZE);
    uECC_word_t *l_result = malloc(num_words * uECC_WORD_SIZE);
    p1[0] = 1;
    l_result[1] = 1;
    for(int x = 1; x < num_words; x++){
        p1[x] = 0;
